...
export async function GET(request: NextRequest) {
  try {
    ...
    // Calculate totals using accountAmountCents (original account currency)
    // For multi-currency, use convertedAmountCents
    const expenses = transactions
      .filter(tx => tx.accountAmountCents < 0)
      .reduce((sum, tx) => sum + Math.abs(tx.accountAmountCents), 0);

    const income = transactions
      .filter(tx => tx.accountAmountCents > 0)
      .reduce((sum, tx) => sum + tx.accountAmountCents, 0);
    ...
    transactions
      .filter(tx => tx.accountAmountCents < 0 && tx.categoryId)
      .forEach(tx => {
        ...
        categoryExpenses.set(tx.categoryId!, {
          ...current,
          amount: current.amount + Math.abs(tx.accountAmountCents),
        });
      });
    ...
    transactions
      ...
      .forEach(tx => {
        ...
        dailyTrend.set(dayKey, current + Math.abs(tx.accountAmountCents));
      });
